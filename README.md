...
A research project exploring entropy dynamics during reinforcement learning training, with a focus on understanding the theoretical foundations and empirical behavior of entropy changes in policy optimization. The original motivation was to understand what kind of sequences/tokens used in updates have especially large effect on the change in entropy, and whether we can characterize sequences that have a big effect on the entropy but not as much on the expected reward - in other words, are there sequences or tokens for which we can adjust the loss so that the entropy decreases less but at small cost to the change in expected reward. Or put even more simply, how can we encourage more exploration at small local cost to performance? Doing this probably helps performance overall down the line.

However, I've found myself mostly focusing on how to do better "experiment design". Doing reliable measurements and getting results that I can trust turned out to be more difficult than I expected. To start I focused on showing that for small enough learning rates, hopefully including those I used during training, the change in entropy over a training step was approximately linear in the learning rate. Being able to take advantage of this linearized approximation would be crucial later, so it was important to show this very reliably. 

To do these measurements, I wanted to take an approach that was efficient, flexible, and scalable. What I ended up deciding to do was to do a small training run to produce a series of model checkpoints with optimizer states. From each of these, we can do repeated measurements (for example, what is the entropy change after an update $\theta \rightarrow \theta +\eta*v$ for different learning rates $\eta$). In order to get clean and reliable results, I had to be very careful with my estimators of the entropy and its gradients, using variance reduction techniques like Rao-Blackwellization and control variates. With this approach I was successfully able to confirm the validity of the linearized approximation for the learning rates I used in training. Below I have some sample plots as well as more explanation of the theory and my methods. See also [`entropy_experiments/README.md`](entropy_experiments/README.md) for more discussion of the experimental methods and motivation behind them.

With the validity of the linearized approximation established, I am now doing various experiments to explore the correlation between the contribution of individual samples on the entropy (which can be isolated cleanly at linear order in the learning rate) and properties of those sequences. 
