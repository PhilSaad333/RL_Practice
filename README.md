...
**A research project exploring entropy dynamics during reinforcement learning training, with a focus on understanding the theoretical foundations and empirical behavior of entropy dynamics in policy optimization.** 

Entropy typically decreases during RL training, reflecting the exploration/exploitation tradeoff. The results of https://arxiv.org/abs/2505.22617 suggest that some tokens/samples have an outsized contribution to this decrease in entropy. This motivates the question **"For ordinary policy gradient algorithms, how do properies of the samples used in a training step correlate with the entropy change from that step?"** A better understanding of this may be useful in designing RL algorithms that manage encourage more exploration of diverse policies further into training.

To get reliable results we need to be able to do precise measurements of the change in entropy from a training step and do repeated controlled measurements. So far a large portion of the time I've spent on this project (after initial setup such as making sure my model training setup is working so that we have a good 'laboratory' for studying the problem) involved learning better **"experiment design"** - how should I organize my experiments, how can I use variance reduction techniques to get clean measurements, how can I balance the need for the need for fairly high numerical precision with practical considerations, etc.

Based on some theoretical studies of the entropy dynamics (see [`docs/RL_studies.pdf`](docs/RL_studies.pdf)), I decided that testing whether the entropy change was approximately linear in the learning rate for the learning rates I used in training was an important first step. If this approximation is valid for learning rates of interest, then the contributions of individual samples/tokens to the entropy change can be isolated (as the entropy change is approximately a sum over seperate contributions from each sample), and can be related to a basic object of more general interest, the "neural tangent kernel" or "Fisher kernel". So my first goal was to do precision measurements of the change in entropy over a training step (see the **Results So Far** section below) to test this approximation.

My basic approach was to start by producing a series of checkpoints (with optimizer states) from a small training run. For each of these, we can do any number of "simulated" update steps, swept over learning rates. For a given batch of samples used to do a training step, we compute the "update vector" (the change in parameters normalized by the learning rate), using the optimizer state so we can reconstruct the true AdamW update. Then we can run the model with paramters $\theta + \eta*v$ swept over learning rates to compute the entropy before and after a step (to get accurate measuments of the entropy we used various variance reduction technqiues, and an importance sampling correction). We also explicitly compute the linearized formula for the change in entropy $\mathcal{H}$, $\delta\mathcal{H}_{linear} = \eta * \nabla \mathcal{H} \cdot v$ (note that by explicitly computing the update vector $v$ we can compute this, the true change in entropy swept over learning rates, and a variety of other things by only having to do one gradient computation). **We then compare the true entropy change, swept over learning rate, to a linear fit, and to the linearized approximation as a sanity check, and find that the linearized approximation holds well for the learning rates of interest.** See also [`entropy_experiments/README.md`](entropy_experiments/README.md) for more discussion of the experimental methods and motivation behind them.

**A lot of the time spent on getting those results involved learning and practicing with various variance reduction technqiues, such as Rao-Blackwell estimators and control variates, and finding places where my accuracy was limted by numerical precision.** My initial results were pretty bad, but in the end I'm pretty happy with my results. I'm surprised how important these (what seemed at first to be a bit esoteric) variance reduction methods are!

Currently I'm doing variations of these precision measurements of the entropy change designed to isolate the effects of individual samples. For example, we can decompose the update vector as a sum over the AdamW weight decay and momentum terms and contributions from each individual sequence/token $v = v_{baseline} + \sum_{i\in B} v_i$. In principle, when this linearized in learning rate approximation holds, the entropy change is a sum over contributions from each of these components, $\delta \mathcal{H} \approx \delta\mathcal{H}_{baseline} +  \sum_{i\in B} \delta\mathcal{H}_i$. **Studying the correlation between the $\delta\mathcal{H}_i$ and properties of the sequence $i$ is my current main goal.** However, my measurements of the $\delta \mathcal{H}_i$ are noisier than my measurements of the full $\delta \mathcal{H}$, so currently I am working on improving those measurements with a sanity check being to demonstrate that they sum to the full entropy change.
