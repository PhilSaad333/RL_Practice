...
Currently I'm doing variations of these precision measurements of the entropy change designed to isolate the effects of individual samples. For example, we can decompose the update vector as a sum over the AdamW weight decay and momentum terms and contributions from each individual sequence/token $v = v_{baseline} + \sum_{i\in B} v_i$. In principle, when this linearized in learning rate approximation holds, the entropy change is a sum over contributions from each of these components, $\delta H \approx \delta H_{baseline} +  \sum_{i\in B} \delta H_i$. **Studying the correlation between the $\delta H_i$ and properties of the sequence $i$ is my current main goal.** However, my measurements of the $\delta H_i$ are noisier than my measurements of the full $\delta H$, so currently I am working on improving those measurements with a sanity check being to demonstrate that they sum to the full entropy change.
