...
│   ├── results.json      # Probe JSON results
│   ├── config.yaml       # Exact config used  
│   └── error.txt         # If run failed
├── summary.json          # Final convergence analysis

Usage:
    python flexible_batch_test.py --checkpoint /path/to/checkpoint
    python flexible_batch_test.py --be-values 16,32,64 --runs 3 --config /path/config.yaml
"""
...
def setup_run_directory(results_base: Path, B_E: int, run_id: int) -> Path:
    """Create isolated directory for a single run."""
    run_dir = results_base / f"BE{B_E:03d}_run{run_id:03d}"
...
def create_run_config(base_config_path: str, run_dir: Path, B_E: int, B_U: int) -> str:
    ...
    # Set results path to this run's directory
    config['output']['results_path'] = str(run_dir / 'results.json')
...
def main():
    parser = argparse.ArgumentParser(description="Flexible Batch Size Test for Entropy Probe")
    ...
    print("🧪 FLEXIBLE BATCH SIZE TEST")
    ...
    for B_E in B_E_values:
        ...
        for run_id in range(1, args.runs + 1):
            ...
            run_dir = setup_run_directory(results_base, B_E, run_id)
    ...
    print(f"\n📁 Complete results saved to: {results_base}")
    print(f"📄 Summary: {summary_file}")
    print(f"\nEach run has full logs in its BE*_run*/ directory")
