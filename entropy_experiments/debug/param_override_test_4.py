#!/usr/bin/env python3
"""
Fixed-Sequence Param-Override Sanity Probe (restored)
=====================================================

- Loads model via OfflineEntropyProbe and LoRA checkpoint
- Builds an update vector v_named from a tiny U-batch (compute_rb=False)
...
Run:
  python param_override_test_2.py --config <path/to/colab_config.yaml> \
         --tokenizer_id Qwen/Qwen2.5-1.5B-Instruct \
         --eta_grid 0 1e-10 1e-8 3e-8 1e-7 3e-7 1e-6 3e-6 1e-5
Add --fp64 if you want a wider linear range.
...
# --- small helpers ------------------------------------------------------------
def unwrap_ddp_keep_peft(mod: torch.nn.Module) -> torch.nn.Module:
    """Unwrap DDP only; keep PEFT/LoRA wrapper intact."""
    return mod.module if hasattr(mod, "module") else mod

def device_of(mod: torch.nn.Module) -> torch.device:
    return next(unwrap_ddp_keep_peft(mod).parameters()).device
...
def _params_only_fp(m: torch.nn.Module, v_named, eta: float, dtype):
    """Build params-only mapping keyed to THIS module in requested dtype."""
    params, _ = build_functional_params_named(
        m, v_named, eta,
        detach_params=True, detach_buffers=True,
        force_param_dtype=dtype,       # dtype is torch.float32/64 or None
        force_buffer_dtype=None
    )
    return params

def _params_plus_buffers_fp(m: torch.nn.Module, v_named, eta: float, dtype):
...
def main():
    ...
    # Avoid re-enabling TF32 through any helper
    try:
    ...
    model = probe.model
    dev = device_of(model)
    print(f"✓ Model on {dev}: {type(unwrap_ddp_keep_peft(model)).__name__}")

    mdl_target = unwrap_ddp_keep_peft(model)  # use this EVERYWHERE
    ...
    v_named, _ = compute_update_vector(
    ...
    # Baseline mapping at η=0 for the PROBE (same dtype policy as all η>0)
    map0_fp = _params_only_fp(mdl_target, v_named=None, eta=0.0, dtype=fp_dtype)
    logits0 = _fc_logits_noautocast(mdl_target, input_ids, attention_mask, map0_fp).detach().cpu()

    # Optional separate diagnostic: “direct vs fc(η=0)” with preserved dtype
    map0_preserve = _params_only_fp(mdl_target, v_named=None, eta=0.0, dtype=None)
    with torch.no_grad():
        with torch.autocast(device_type="cuda", enabled=False):
    ...
    logits0_pres = _fc_logits_noautocast(mdl_target, input_ids, attention_mask, map0_preserve).detach().cpu()
    print(f"[zero-check] max|direct - fc_preserve(η=0)| = {(logits_dir - logits0_pres).abs().max().item():.3e}")
    ...
    # Δθ scaling check on mappings (FP policy identical to the probe)
    p_main = _params_only_fp(mdl_target, v_named, eta=1e-6, dtype=fp_dtype)
    p_tiny = _params_only_fp(mdl_target, v_named, eta=1e-8, dtype=fp_dtype)
    def l2_of_delta(pa, pb):
        ...
        return s ** 0.5
    r = l2_of_delta(p_tiny, map0_fp) / l2_of_delta(p_main, map0_fp)
    print(f"||Δθ|| ratio (tiny/main): {r:.3e}  (expected ≈ 1e-2 for 1e-8 vs 1e-6)")

    # --- PROBE A: PARAMS-ONLY mapping (recommended) --------------------------
    print("\n[PROBE A] PARAMS-ONLY mapping (recommended)")
    records_A = []
    for eta in args.eta_grid:
        m_eta = map0_fp if eta == 0.0 else _params_only_fp(mdl_target, v_named, eta, dtype=fp_dtype)
    ...
    records_B = []
    for eta in args.eta_grid:
        mapping = map0_fp if eta == 0.0 else _params_plus_buffers_fp(mdl_target, v_named, eta, dtype=fp_dtype)
