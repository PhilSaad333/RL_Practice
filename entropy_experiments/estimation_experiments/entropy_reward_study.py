...
def run_entropy_study_with_replacement_sampling(
    checkpoint_path: str,
    total_sequences: int,
    dataset_name: str = "gsm8k_r1_template",
    split: str = "train",
    ...
    """
    Run entropy study using independent prompt sampling with replacement.
    
    Uses the new SequenceProcessor.generate_with_replacement_sampling() method
    to achieve true independent sampling from D(p)π(t|p).
    
    Args:
        checkpoint_path: Path to LoRA checkpoint
        total_sequences: Number of sequences to generate (e.g., 4096, 32768)
        dataset_name: Dataset to sample prompts from
        split: Dataset split
        max_prompts_pool: Maximum size of prompt pool (None = use all available)
    ...
    """
    print("=" * 80)
    print("🎲 ENTROPY STUDY WITH REPLACEMENT SAMPLING")
    print("=" * 80)
    ...
    # Generate sequences using replacement sampling
    print("🔄 Generating sequences with replacement sampling...")
    generation_start = datetime.now()
    
    sequences, logprob_results, diagnostics_results = processor.generate_with_replacement_sampling(
        total_sequences=total_sequences,
        dataset_name=dataset_name,
        split=split,
        max_prompts_pool=max_prompts_pool,
        seed=seed,
        with_grad=False,
        compute_rb=True
    )
    ...
    generation_duration = (datetime.now() - generation_start).total_seconds()
    print(f"✅ Generation completed in {generation_duration:.1f}s ({generation_duration/60:.1f} min)")
    print(f"⚡ Throughput: {total_sequences/generation_duration:.1f} sequences/second")
    print()
    ...
    results = analyze_entropy_results(
        sequences, logprob_results, diagnostics_results,
        checkpoint_path, total_sequences, generation_duration,
        dataset_name, split, temperature, top_p, max_new_tokens,
        gen_batch_size, tf_batch_size
    )
    
    # Add replacement sampling metadata
    results["experiment_info"]["sampling_method"] = "independent_with_replacement"
    results["experiment_info"]["total_sequences"] = total_sequences
    results["experiment_info"]["max_prompts_pool"] = max_prompts_pool
    results["experiment_info"]["sampling_seed"] = seed
    ...
    results["timing"] = {
        ...
        "generation_duration_seconds": generation_duration,
        "analysis_duration_seconds": analysis_duration,
        "sequences_per_second": total_sequences / generation_duration,
        "start_time": start_time.isoformat(),
        "end_time": datetime.now().isoformat()
    }
...
def run_colab_replacement_study(
    ...
    """Convenience function for Colab execution with replacement sampling."""
    
    checkpoint_path = "/content/drive/MyDrive/RL_Practice_Files/new_rl_checkpoint/step_60/model"
    
    print(f"🎲 Running REPLACEMENT SAMPLING entropy study on Google Colab A100")
    print(f"🎯 {total_sequences:,} sequences via independent prompt sampling")
    print(f"🔧 Config: temp={temperature}, top_p={top_p}, gen_batch={gen_batch_size}, tf_batch={tf_batch_size}")
    
    return run_entropy_study_with_replacement_sampling(
        checkpoint_path=checkpoint_path,
        total_sequences=total_sequences,
        temperature=temperature,
        max_new_tokens=max_new_tokens,
